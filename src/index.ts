...
function isNativeCompactionEntry(entry: unknown): entry is NativeCompactionEntry {
  const e = entry as NativeCompactionEntry | undefined;
  return Boolean(
    e &&
      e.type === "compaction" &&
      e.details &&
      (!e.details.strategy || e.details.strategy === NATIVE_COMPACTION_STRATEGY) &&
...
function isPiCompactionEntry(entry: unknown): entry is PiCompactionEntry {
  ...
  return Boolean(e && e.type === "compaction" && typeof e.summary === "string" && e.summary.trim().length > 0);
}

function findLatestPiCompaction(branch: readonly unknown[]): { entry: PiCompactionEntry; index: number } | undefined {
  for (let i = branch.length - 1; i >= 0; i--) {
    const entry = branch[i];
...
function runPiMemorySession(args: { cwd: string; config: Config; compactedWindow: unknown[]; summaryText?: string; tailText: string }): Promise<string> {
  ...
  writeFileSync(payloadPath, JSON.stringify({ compactedWindow: args.compactedWindow, userContent }));
  ...
  if (args.config.thinking && args.config.thinking !== "off") cliArgs.push("--thinking", args.config.thinking);
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      child.kill("SIGKILL");
      reject(new Error(`memory session timed out after ${args.config.timeoutMs}ms`));
    ...
    child.on("close", (code) => {
      ...
      else reject(new Error(`pi memory session exited ${code}: ${stderr.trim()}`));
    });
  });
}

function appendInbox(config: Config, ctx: ExtensionContext, content: string): void {
...
export default function piMemories(pi: ExtensionAPI) {
  ...
  pi.on("session_shutdown", async (_event, ctx) => {
    ...
    if (latest) {
      logDebug(config, `native compaction found index=${latest.index} id=${latest.entry.id} window=${latest.entry.details.compactedWindow.length}`);
    } else if (latestPi) {
      logDebug(config, `pi compaction found index=${latestPi.index} id=${latestPi.entry.id} summaryChars=${latestPi.entry.summary.length}`);
    } else {
      logDebug(config, "no native compaction entry found; using branch tail only");
    }
    ...
    const tailEntries = tailStartIndex === undefined ? branch : branch.slice(tailStartIndex + 1);
    if (!latest && !latestPi) {
      const userMessages = countUserMessages(branch);
      if (userMessages < config.minUserMessagesWithoutBlob) {
        logDebug(config, `short no-blob session skipped userMessages=${userMessages} min=${config.minUserMessagesWithoutBlob}`);
        return;
      }
    }
    ...
    try {
      logDebug(config, `spawning memory session blobItems=${compactedWindow.length} tailChars=${tailText.length}`);
